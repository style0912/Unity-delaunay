...
//			}
		...
		*/
        public static List<LineSegment> Kruskal(List<LineSegment> lineSegments, KruskalType type) {
            ...
            for (int i = lineSegments.Count; --i > -1;) {
                ...
                if (rootOfSet0 != rootOfSet1)
                    ...
                    // merge the two sets:
                    if (rootOfSet0.treeSize >= rootOfSet1.treeSize) {
                        // set0 absorbs set1:
                        if(!rootNodes.Contains(node1))
                            rootNodes.Add(node1);
                        rootOfSet1.parent = rootOfSet0;
                        rootOfSet0.treeSize += rootOfSet1.treeSize;
                    }
                    else {
                        // set1 absorbs set0:
                        if (!rootNodes.Contains(node0))
                            rootNodes.Add(node0);
                        rootOfSet0.parent = rootOfSet1;
                        rootOfSet1.treeSize += rootOfSet0.treeSize;
                    }
                }
            }
            ...
            //    foreach (var element in mst) {
            //        bool flag = false;
            //        foreach (var element2 in mst) {
            //            if (element.Equals(element2))
            //                continue;
            //            if (element.p0 == element2.p0) {
            //                flag = true;
            //                break;
            //            }
            //            if (element.p0 == element2.p1) {
            //                flag = true;
            //                break;
            //            }
            //        }
            //        if (!flag) {
            //            endPoints.Add(element.p0);
            //        }
            ...
            //            if (element.Equals(element2))
            //                continue;
            //            if (element.p1 == element2.p0) {
            //                flag = true;
            //                break;
            //            }
            //            if (element.p1 == element2.p1) {
            //                flag = true;
            //                break;
            //            }
            //        }
            //        if (!flag) {
            //            endPoints.Add(element.p1);
            //        }
            //    }
                
            //    foreach (var element in endPoints) {
            //        var query = lineSegments.FindAll(x => (x.p0.Value.Equals(element.Value) || x.p1.Value.Equals(element.Value)) && !mst.Contains(x));
            //        if (query.Any()) {
            //            if (type == KruskalType.MINIMUM)
            //                mst.Add(query.OrderBy(x => Vector2.Distance(x.p0.Value, x.p1.Value) * (nodes[x.p0].treeSize * 0.1f)).First());
            //            else mst.Add(query.OrderBy(x => Vector2.Distance(x.p0.Value, x.p1.Value) * (nodes[x.p0].treeSize * 0.1f)).Last());
            //        }
            //    }
            //}
