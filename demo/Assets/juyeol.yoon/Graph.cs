...
namespace style0912 {
    public class Graph {
        ...
        public Graph(Random seed, IEnumerable<Vector2> points, Voronoi voronoi, int width, int height, float lakeThreshold) {

            Width = width;
            Height = height;

            inside = Shape.MakePerlin(seed);

            BuildGraph(points, voronoi);
            AssignCornerElevations(seed);
        ...
        private void BuildGraph(IEnumerable<Vector2> points, Delaunay.Voronoi voronoi) {
            ...
            foreach (var libedge in libedges) {
                ...
                if (vedge.p0.HasValue && vedge.p1.HasValue)
                    edge.midpoint = Vector2Extend.Interpolate(vedge.p0.Value, vedge.p1.Value, 0.5f);
        ...
        private void AssignCornerElevations(style0912.Random seed) {
        ...
        private void CreateRivers(style0912.Random seed) {
            // Create rivers along edges. Pick a random corner point, then
            // move downslope. Mark the edges and corners as rivers.
            for (var i = 0; i < (Width + Height) / 4; i++) {
                var q = corners[seed.Next(0, corners.Count)];
        ...
        static Biome GetBiome(Center p) {
            if (p.ocean) {
                ...
                if (p.elevation < 0.1f) return Biome.Marsh;
                if (p.elevation > 0.8f) return Biome.Ice;
                return Biome.Lake;
            }
            else if (p.coast) {
                return Biome.Beach;
                ...
                if (p.moisture > 0.5f) return Biome.Snow;
                else if (p.moisture > 0.33f) return Biome.Tundra;
                else if (p.moisture > 0.16f) return Biome.Bare;
                else return Biome.Scorched;
            }
            else if (p.elevation > 0.6f) {
                if (p.moisture > 0.66f) return Biome.Taiga;
                else if (p.moisture > 0.33f) return Biome.Shrubland;
                else return Biome.TemperateDesert;
            }
            else if (p.elevation > 0.3f) {
                if (p.moisture > 0.83f) return Biome.TemperateRainForest;
                else if (p.moisture > 0.5f) return Biome.TemperateDeciduousForest;
                else if (p.moisture > 0.16) return Biome.Grassland;
                else return Biome.TemperateDesert;
            }
            else {
                if (p.moisture > 0.66f) return Biome.TropicalRainForest;
                else if (p.moisture > 0.33f) return Biome.TropicalSeasonalForest;
                else if (p.moisture > 0.16f) return Biome.Grassland;
                else return Biome.SubtropicalDesert;
            }
        }

        public Texture2D GenerateTexture(int textureScale, bool drawEdges) {
            Texture2D texture = new Texture2D(Width * textureScale, Height * textureScale);
            texture.SetPixels(Enumerable.Repeat(Color.magenta, texture.width * texture.height).ToArray());

            var lines = edges.Where(p => p.v0 != null).Select(p => new[]
            {
            ...
            foreach (var c in centers)
                texture.FillPolygon(c.corners.Select(p => new Vector2(p.point.x * textureScale, p.point.y * textureScale)).ToArray(), BiomeProperties.Colors[c.biome]);

            if(drawEdges)
                foreach (var line in lines)
                    DrawLine(texture, line[0], line[1], line[2], line[3], textureScale, Color.black);

            foreach (var line in edges.Where(p => p.river > 0 && !p.d0.water && !p.d1.water))
                DrawLine(texture, line.v0.point.x, line.v0.point.y, line.v1.point.x, line.v1.point.y, textureScale, Color.blue);
        ...
        private void DrawLine(Texture2D texture, float x0, float y0, float x1, float y1, int textureScale, Color color) {
